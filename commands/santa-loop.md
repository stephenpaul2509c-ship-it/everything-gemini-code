...
Run two independent reviewers (Gemini gemini-2.5-pro + an external model) against the current task output. Both must return NICE before the code is pushed. If either returns NAUGHTY, fix all flagged issues, commit, and re-run fresh reviewers — up to 3 rounds.
...
Launch an Agent (subagent_type: `code-reviewer`, model: `gemini-2.5-pro`) with the full rubric + all files under review. The prompt must include:
- The complete rubric
...
Reviewer A (Gemini gemini-2.5-pro):   [PASS/FAIL]
...
- Reviewer A (Gemini gemini-2.5-pro) always runs — guarantees at least one strong reviewer regardless of tooling.
...
- Strongest available models are used: gemini-2.5-pro for Reviewer A, GPT-5.4 or Gemini 2.5 Pro for Reviewer B.
