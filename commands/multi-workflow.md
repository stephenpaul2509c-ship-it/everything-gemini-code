...
- Multi-model collaboration: Codex (backend) + Antigravity (frontend) + Gemini (orchestration)
...
# New session call
Bash({
  command: "~/.gemini/bin/codeagent-wrapper {{LITE_MODE_FLAG}}--backend <codex|antigravity> - \"$PWD\" <<'EOF'
...
# Resume session call
Bash({
  command: "~/.gemini/bin/codeagent-wrapper {{LITE_MODE_FLAG}}--backend <codex|antigravity> resume <SESSION_ID> - \"$PWD\" <<'EOF'
ROLE_FILE: <role prompt path>
<TASK>
Requirement: <enhanced requirement (or $ARGUMENTS if not enhanced)>
Context: <project context and analysis from previous phases>
...
|-------|-------|--------|
| Analysis | `~/.gemini/.ccg/prompts/codex/analyzer.md` | `~/.gemini/.ccg/prompts/antigravity/analyzer.md` |
| Planning | `~/.gemini/.ccg/prompts/codex/architect.md` | `~/.gemini/.ccg/prompts/antigravity/architect.md` |
| Review | `~/.gemini/.ccg/prompts/codex/reviewer.md` | `~/.gemini/.ccg/prompts/antigravity/reviewer.md` |
...
**Gemini Synthesis**: Adopt Codex backend plan + Antigravity frontend plan, save to `.gemini/plan/task-name.md` after user approval.
