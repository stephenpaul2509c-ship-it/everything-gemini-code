---
description: Generate a local Gemini CLI / Antigravity cost report from the EGC cost-tracker metrics log.
argument-hint: [csv]
---

# Cost Report

Summarize local Gemini CLI / Antigravity spend by day, model, and session from the metrics
log that EGC's `stop:cost-tracker` hook writes.
...
1. Check that `~/.gemini/metrics/costs.jsonl` exists. If it does not, tell the
