---
description: Detect a project's stack and produce a dry-run EGC onboarding plan using the repository's install manifests and stack mappings.
---

# /project-init

Create a safe, reviewable EGC onboarding plan for the current project. This command should start in dry-run mode and only write files after explicit user approval.
...
1. Default to dry-run. Do not modify `GEMINI.md`, settings files, rules, skills, or install state until the user approves the concrete plan.
2. Preserve existing project guidance. If `GEMINI.md`, `.gemini/settings.local.json`, `.cursor/`, `.codex/`, `.gemini/`, `.opencode/`, `.codebuddy/`, `.joycode/`, or `.qwen/` already exists, inspect it and propose a merge/append plan instead of overwriting.
3. Use EGC's installer and manifest tooling. Do not hand-copy files or clone arbitrary remotes as an install shortcut.
...
If the user wants a `GEMINI.md` starter, generate it separately from the installer plan and keep it minimal:
...
Never replace an existing `GEMINI.md` without showing a diff and receiving approval.
