...
Create a skill at `~/.gemini/skills/<pattern-name>/SKILL.md`:
...
- Validate `pattern-name` as a lowercase hyphenated slug. Reject path
  ...
  inside the approved skill root (`~/.gemini/skills/`).
...
5. Save to `~/.gemini/skills/<pattern-name>/SKILL.md`
...
The directory form and frontmatter matter because Gemini CLI / Antigravity discovers
personal skills from `<name>/SKILL.md`; a flat `skills/learned/<name>.md` file
is not a skill entrypoint. The trigger-first description helps Gemini decide
