---
name: delivery-gate
description: Stop hook that blocks Gemini from finishing until quality checks pass. Detects rationalization patterns (surface text heuristics), stale learning logs (filesystem mtime), and low disk space. Complements self-audit by mechanically enforcing learning capture habits. Use when Gemini should be mechanically blocked from declaring work finished before quality checks and learning capture actually pass.
metadata:
  version: 1.1.1
  origin: EGC
---

# Delivery Gate — Mechanical Quality Gate for Gemini CLI / Antigravity

A **Stop hook** that checks three things before Gemini can finish a session, using only **deterministic checks** — file modification timestamps, disk usage, and regex patterns on the transcript text. No AI inference.
...
Gemini CLI / Antigravity's built-in checks cover code quality (build → type → lint → test). But there's a different failure mode: the agent produces working code while the **session hygiene was neglected** — learning not captured, rationalized shortcuts, disk running out silently.
...
```bash
cp quality-gate.py ~/.gemini/scripts/
```
...
```json
{
  "hooks": {
    "Stop": [{
      "hooks": [{
        "type": "command",
        "command": "python3 ~/.gemini/scripts/quality-gate.py",
        "timeout": 5000
      }]
    }]
  }
