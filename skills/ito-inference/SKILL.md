...
`ito-inference` is the sole canonical EGC skill for inference serving on Itô
compute. Requests naming `ito-serve` route here; do not create or install a
second `ito-serve` skill. EGC never SSHes to nodes, downloads weights, launches
an engine, or exposes an endpoint; it never books, reserves, or spends.

## Current production boundary

Managed serving is unavailable today. The EGC bridge exposes only `login`,
...
probe; EGC does not invoke or depend on it. The MCP surface exposes only auth,
find, and status. The locally enforceable guarantee is that EGC rejects `serve`
...
The canonical API—not EGC—must own workload creation and return structured JSON
...
These stages describe the future backend, not code that exists in EGC:
...
4. Launch through the provider control plane, never direct root SSH from EGC.
