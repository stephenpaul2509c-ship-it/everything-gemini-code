---
name: agentic-os
description: Build persistent multi-agent operating systems on Gemini CLI / Antigravity. Covers kernel architecture, specialist agents, slash commands, file-based memory, scheduled automation, and state management without external databases. Use when building a persistent multi-agent system on Gemini CLI / Antigravity with its own memory, commands, and scheduling.
metadata:
  origin: EGC
---
...
Treat Gemini CLI / Antigravity as a persistent runtime / operating system rather than a chat session. This skill codifies the architecture used by production agentic setups: a kernel config that routes tasks to specialist agents, persistent file-based memory, scheduled automation, and a JSON/markdown data layer.

## When to Activate

- Building a multi-agent workflow inside Gemini CLI / Antigravity
- Setting up persistent Gemini CLI / Antigravity automation that survives session restarts
...
```
project-root/
├── GEMINI.md          # Kernel: identity, routing rules, agent registry
├── agents/            # Specialist agent definitions (markdown prompts)
├── .gemini/commands/  # Slash commands: user-facing CLI
...
|---|---|---|
| Kernel (`GEMINI.md`) | Identity, routing, model policies, agent registry | Git-tracked |
| Agents (`agents/`) | Specialist identities with scoped tools and memory | Git-tracked |
| Commands (`.gemini/commands/`) | User-facing slash commands (`/daily-sync`, `/outreach`) | Git-tracked |
...
`GEMINI.md` is the kernel. It acts as the COO / orchestrator. Gemini reads it at session start and uses it to route work.

### Kernel Structure

```markdown
# GEMINI.md - Agentic OS Kernel
...
Each agent is a standalone markdown file in `agents/`. Gemini loads the relevant agent file when routing a task.
...
- Test runner access
- MCP servers as configured in `.gemini/mcp.json`
...
For parallel execution, use Gemini CLI / Antigravity's background task capability or shell scripts that invoke Gemini CLI / Antigravity with specific agent contexts.

## Commands and Daily Workflows

Slash commands are markdown files in `.gemini/commands/`. They define reusable workflows.
...
Place command files in `.gemini/commands/<command-name>.md`. Gemini CLI / Antigravity auto-discovers them. Users invoke them with `/<command-name>`.
...
Agentic OS tasks run on a schedule using external cron, not Gemini CLI / Antigravity's built-in cron (which dies when the session ends).
...
Starting fresh every time Gemini CLI / Antigravity opens.
```
...
```markdown
# BAD - API keys in agent files or GEMINI.md
...
Keep routing declarative in `GEMINI.md` markdown tables. It is inspectable, editable, and debuggable.

## Best Practices

- [ ] `GEMINI.md` is under 200 lines and fits in context window
...
- [ ] Scheduled tasks use external cron (LaunchAgent, systemd, pm2), not Gemini CLI / Antigravity's session cron
