...
Use the canonical Itô compute CLI or MCP server. EGC does not implement a
parallel client, local simulation, reservation, workload runner, or inference
server. EGC itself does no browser automation.
...
EGC never discovers this credential-bearing client through `PATH`.
...
1. Run `ecc ito login` before the first operation. EGC delegates this to the
   ...
   `ecc ito login --no-browser` to suppress the page handoff. EGC itself does no
...
recover, repair, reset, purchase, or order resources. EGC does not forward
...
order. The passive dashboard link in EGC help is a separate user-operated web
