...
Use this skill when a user explicitly asks EGC to install, inspect, or remove
...
- Install only an EGC-qualified pinned version, currently `v0.1.0`.
...
- Remove only a still-qualified EGC-managed binary with
...
The initial EGC bridge supports qualified installation, read-only status, and
ownership-checked uninstall. Use the canonical Nasiko CLI directly for connection, authentication,
launch, deployment, or shutdown until those verbs have their own verified EGC
...
- Do not search arbitrary `PATH` entries. Use EGC's qualified location or an
