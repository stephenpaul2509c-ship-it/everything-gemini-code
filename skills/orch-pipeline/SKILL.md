...
each phase to an existing EGC agent or command. This file is that pipeline.
...
> These wrappers **compose** existing EGC commands rather than replace them:
...
Match the language reviewer to the repo (see the repo's own `GEMINI.md`).
