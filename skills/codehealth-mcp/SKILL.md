...
**Opt-in (EGC):** The `codescene` block in `mcp-configs/mcp-servers.json` is a template only. EGC plugin installs do not auto-enable bundled MCP servers. Copy the entry into your config only if you want it. You can exclude it during EGC install/sync with `ECC_DISABLED_MCPS=codescene,...`.
...
Same triggers as **When to Use** above — this heading is what EGC uses for skill auto-activation.
...
**Gemini CLI / Antigravity** (`~/.gemini.json` → `mcpServers`):
...
Paste into the project `AGENTS.md` or `GEMINI.md`:
...
| EGC skill / flow | Code Health MCP role |
|------------------|----------------------|
...
**Context tip:** EGC recommends keeping MCP count low. Enable `codescene` when doing substantive edits; disable when not needed.
