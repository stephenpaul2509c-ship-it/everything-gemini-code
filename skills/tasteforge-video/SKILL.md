...
`tasteforge` package in the Itô video repository; EGC orchestrates and
...
This boundary is the core of the skill. Everything EGC can actually run is
**local, deterministic, and offline**:

| Operation | Deterministic? | EGC may run |
|---|---|---|
...
**Provider generation must fail closed in EGC.** Any live Fal (or other
...
separate lane with its own review. EGC never calls Fal, never reads any API
...
EGC's job is to route here, run the local deterministic commands, and
