...
description: "Patterns and architectures for autonomous Gemini CLI / Antigravity loops — from simple sequential pipelines to RFC-driven multi-agent DAG systems. Retained for compatibility only: when new autonomous loop guidance is needed, use continuous-agent-loop instead."
metadata:
  origin: EGC
---
...
### Implementation via Gemini CLI / Antigravity Commands
...
**A production-grade shell script** that runs Gemini CLI / Antigravity in a continuous loop, creating PRs, waiting for CI, and merging automatically. Created by AnandChowdhary (credit: @AnandChowdhary).
...
| Stage | Model | Purpose |
|-------|-------|---------|
| Research | gemini-2.5-pro | Read codebase + RFC, produce context doc |
| Plan | gemini-2.5-pro | Design implementation steps |
| Implement | Codex | Write code following the plan |
| Test | gemini-2.5-pro | Run build + test suite |
| PRD Review | gemini-2.5-pro | Spec compliance check |
| Code Review | gemini-2.5-pro | Quality + security check |
| Review Fix | Codex | Address review issues |
| Final Review | gemini-2.5-pro | Quality gate (large tier only) |
