...
# Gemini CLI / Antigravity passes hook data via stdin as JSON.
#
...
# wrapper's positional phase argument, but Gemini CLI / Antigravity still exposes the hook
# event name in GEMINI_HOOK_EVENT_NAME.  Fall back to that env var before
...
if [ -z "$HOOK_PHASE" ]; then
  case "${GEMINI_HOOK_EVENT_NAME:-}" in
...
# Read JSON from stdin (Gemini CLI / Antigravity hook format)
INPUT_JSON=$(cat)
...
if [ -n "$STDIN_CWD" ] && [ -d "$STDIN_CWD" ]; then
  ...
  if [ -n "$_GIT_ROOT" ]; then
    export GEMINI_PROJECT_DIR="$_GIT_ROOT"
...
# Pass HOOK_PHASE via env var since Gemini CLI / Antigravity does not include hook type in stdin JSON
...
try:
    ...
    # Gemini CLI / Antigravity does NOT include a "hook_type" field in the stdin JSON,
    # so we rely on the shell argument ("pre" or "post") instead.
    hook_phase = os.environ.get("HOOK_PHASE", "post")
    event = "tool_start" if hook_phase == "pre" else "tool_complete"

    # Extract fields - Gemini CLI / Antigravity hook format
