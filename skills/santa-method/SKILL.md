...
```python
# Spawn reviewers in parallel (Gemini CLI / Antigravity subagents)
...
### Pattern A: Gemini CLI / Antigravity Subagents (Recommended)

Subagents provide true context isolation. Each reviewer is a separate process with no shared state.

```bash
# In a Gemini CLI / Antigravity session, use the Agent tool to spawn reviewers
