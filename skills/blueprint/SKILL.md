...
4. **Review** — Delegates adversarial review to a strongest-model sub-agent (e.g., gemini-2.5-pro) against a checklist and anti-pattern catalog. Fixes all critical findings before finalizing.
...
- **Zero runtime risk** — Pure Markdown skill. The entire repository contains only `.md` files — no hooks, no shell scripts, no executable code, no `package.json`, no build step. Nothing runs on install or invocation beyond Gemini CLI / Antigravity's native Markdown skill loader.
...
- Gemini CLI / Antigravity (for `/blueprint` slash command)
