...
description: "Design task-local harnesses, eval gates, and reusable skill extraction for Gemini dynamic workflow mode and other adaptive agent harnesses. Use when building a task-local harness, adding eval gates, or extracting a reusable skill from ad-hoc work."
metadata:
  origin: EGC
---
...
- The user mentions dynamic workflows, custom harnesses, harness-per-task, adaptive workflows, or Gemini CLI / Antigravity dynamic workflow mode.
...
If the repo has ECC2 state enabled, prefer adding or reading checkpoints through the EGC control pane or state-store-backed scripts instead of scattering untracked notes.
