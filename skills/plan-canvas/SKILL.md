...
EGC-native around the `/plan` confirmation gate, with zero dependencies.

## When to Use

- You just wrote a plan artifact (`.gemini/plans/*.plan.md` from `/plan`) and
...
Gemini CLI / Antigravity, a Bash call with `run_in_background: true`). It exits the moment
...
Diagrams render in the EGC dark theme with the accent palette. Mermaid loads in
...
- Markdown artifacts render in EGC's plan template (including Mermaid blocks);
...
- The server is loopback-only and exits after 30 idle minutes
  ...
  in `~/.gemini/plan-canvas/` (`ECC_PLAN_CANVAS_STATE_DIR`).
