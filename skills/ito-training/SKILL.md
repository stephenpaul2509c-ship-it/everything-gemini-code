...
`ito-training` is the canonical EGC skill for training on Itô compute. EGC
...
Managed training is unavailable today. The EGC bridge exposes only `login`,
...
backend remain scaffolds. The locally enforceable guarantee is that EGC rejects
