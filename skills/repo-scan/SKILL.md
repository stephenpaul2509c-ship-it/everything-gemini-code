---
name: repo-scan
description: Bootstrap pointer that installs the external repo-scan skill from a pinned, reviewable commit. Use when repo-scan must be installed before running its cross-stack source-code asset audit; this EGC pointer does not perform the audit itself.
metadata:
  origin: community
---
...
REPO_SCAN_INSTALL_DIR="${GEMINI_CONFIG_DIR:-$HOME/.gemini}/skills/repo-scan"
...
Installation completes only the bootstrap. Reload your agent harness, then invoke `repo-scan` again. This EGC pointer installs the external skill but does not run a scan itself.
