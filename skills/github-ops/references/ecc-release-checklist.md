...
- Milestone: `M0` in the EGC 2.2 release train.
- Contract: ship one exact, verified artifact, keep EGC authority over release
...
## EGC-031 State To Refresh Before Mutating
...
- Do **not** use `scripts/release.sh` as-is for `EGC-031`.
...
git rev-parse HEAD
git rev-parse origin/main
git tag -s "v${VERSION}" -m "EGC ${VERSION}" HEAD
git tag -v "v${VERSION}"
...
required by the checked-in runbook, and verify the native Gemini marketplace
...
### 9. Record Evidence And Close Out EGC-031
...
- Mark `EGC-031` resolved only after the signed patch release is public and
